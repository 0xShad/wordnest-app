...
export default async function Post({ params }: {params: PostProps}) {
  ...
  <div className="max-w-3xl mx-auto py-8 px-4">
    <div className="mb-5">
      <Link href="/" className="font-semibold text-lg" ><ArrowLeft /></Link>
    </div>
    <Card>
      <CardHeader>
        ...
        <div className="flex items-center my-2 gap-4">
          <div className="flex items-center gap-2">
            <img src={data.authorImage} alt={data.authorName} className="rounded-full size-8 object-cover"/>
            <p className="font-semibold text-gray-600 text-sm">{data.authorName}</p>
          </div>
        ...
        <img src={data.imageUrl} alt={data.title} className="rounded-sm object-cover" />
      </CardHeader>
      <CardContent>
